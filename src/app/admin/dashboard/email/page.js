...
export default function AppointmentInboxPage() {
  ...
  const handleStatusUpdate = async (id, newStatus) => {
    ...
    if (!confirm(`Apakah Anda yakin ingin ${actionText} jadwal ini? Email notifikasi otomatis akan dikirim ke klien.`)) return;
    ...
    try {
      ...
      if (res.ok) {
        // Instantly update the UI so the card moves to the correct tab
  ...
  // Helper to format the MongoDB date safely
    ...
    <div className="inbox-container">
      ...
      <header className="inbox-header">
        <div>
          ...
          <p className="inbox-subtitle">Review pengajuan jadwal dari klien dan kirim email konfirmasi secara otomatis.</p>
        </div>
      </header>
          ...
          filteredAppointments.map((app) => (
            <div key={app._id} className="appointment-card glass-panel">
              ...
              <div className="card-body">
                ...
                <div className="info-group">
                  <span className="label">Jadwal Pengajuan</span>
                  <span className="value date-value">
                    {app.schedule ? formatDate(app.schedule.date) : 'Jadwal dihapus'} 
                    <br/>
      ...
      <style jsx>{`
        ...
        .tab-container {
          display: flex; padding: 6px !important; gap: 4px;
        ...
        .tab-btn {
          ...
          color: var(--apple-green-dark); opacity: 0.6;
        ...
        .card-header {
          ...
          border-bottom: 1px solid rgba(26, 71, 42, 0.1); padding-bottom: 16px;
        }
        .card-header h3 { margin: 0; font-size: 18px; color: var(--apple-green-dark); font-weight: 700; }

        .status-tag {
          font-size: 12px; font-weight: 700; padding: 4px 12px; border-radius: 12px; text-transform: uppercase;
        }
        ...
        .service-badge {
          background: var(--apple-green-dark); color: white;
          width: fit-content; padding: 4px 12px; border-radius: 8px; text-transform: capitalize; font-size: 13px;
        }

        .date-value { font-weight: 700; color: var(--apple-green-emerald); }

        .notes-box {
          background: rgba(255,255,255,0.5); padding: 12px; border-radius: 8px; font-style: italic; opacity: 0.8;
        }

        .card-actions {
          display: flex; gap: 12px; margin-top: auto; border-top: 1px solid rgba(26, 71, 42, 0.1); padding-top: 20px;
        }
        ...
        .btn-approve {
          ...
          background: var(--apple-green-emerald); color: white;
          font-weight: 700; cursor: pointer; transition: all 0.2s;
